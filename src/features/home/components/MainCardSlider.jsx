  ...
  return (
    <div className="relative h-[35vh] md:h-[32vh] w-full mx-auto">
      <div className="relative h-full px-0 md:px-6 lg:px-8 md:max-w-[1200px] md:mx-auto">
        {/* 모바일 뷰 */}
        <div className="[@media(max-width:799px)]:block hidden h-full overflow-hidden">
          <div 
        ...
        {/* 데스크탑/태블릿 뷰 */}
        <div className="hidden [@media(min-width:800px)]:flex h-full gap-5 lg:gap-6">
          {cards.map((card, index) => (
            <div 
              key={index}
              className="transition-all duration-300 ease-out cursor-pointer h-full"
              style={{
        ...
        <div className="hidden [@media(min-width:800px)]:flex">
          <div className="absolute inset-y-0 left-0 flex items-center z-10">
            <button
              ...
              className="w-10 h-10 bg-white/80 hover:bg-white rounded-full shadow-md transition-all duration-300 group ml-[-32px]"
              aria-label="이전 슬라이드"
            >
              <div className="w-3 h-3 mx-auto my-auto border-[2px] border-[var(--primary)] border-b-0 border-l-0 rotate-[-135deg] group-hover:border-[var(--point)]" />
            </button>
          </div>
          <div className="absolute inset-y-0 right-0 flex items-center z-10">
            <button
              onClick={handleNextClick}
              className="w-10 h-10 bg-white/80 hover:bg-white rounded-full shadow-md transition-all duration-300 group mr-[-32px]"
              aria-label="다음 슬라이드"
            >
              <div className="w-3 h-3 mx-auto my-auto border-[2px] border-[var(--primary)] border-b-0 border-l-0 rotate-45 group-hover:border-[var(--point)]" />
            </button>
          </div>
        </div>
